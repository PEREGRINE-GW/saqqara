...
if __name__ == "__main__":
    ...
    cross_save_directory = config["simulate"].get("cross_store", "cross_store")
    cross_save_directory = (
        cross_save_directory
        if cross_save_directory[-1] != "/"
        else cross_save_directory[:-1]
    )
    ...
    if not os.path.exists(save_directory + "/" + cross_save_directory):
        os.makedirs(save_directory + "/" + cross_save_directory, exist_ok=True)
    ...
    if (
        ...
        or total_sims(save_directory + "/" + cross_save_directory) != 0
        or total_sims(save_directory + "/" + signal_store) != 0
    ):
        print("[WARNING] Directories not empty, config may not match all simulations")
    print(f"[INFO] Generating {Nsims} simulations")
    try:
        while total_sims(save_directory + "/" + tm_save_directory) < Nsims:
            print("[INFO] Total simulations so far: ", total_sims(save_directory + "/" + tm_save_directory))
            z_out = []
            tm_data_out = []
            oms_data_out = []
            cross_data_out = []
            ...
            for _ in tqdm.tqdm(range(Nsamples_per_chunk), disable=args.q):
                ...
                cg_cross_data = sim.generate_coarse_grained_data_from_sum(
                    2
                    * np.real(
                        np.einsum(
                            "...i,...i->...i",
                ...
                cg_signal_data = sim.generate_coarse_grained_data_from_sum(
                    sample["quadratic_signal_AET"]
                )
                tm_data_out.append(cg_tm_data)
                oms_data_out.append(cg_oms_data)
                cross_data_out.append(cg_cross_data)
                signal_data_out.append(cg_signal_data)
            rid = id_generator(6)
            if len(glob.glob(f"{save_directory + '/' + tm_save_directory}/z_{rid}")) != 0:
                rid = id_generator(6)
            np.save(f"{save_directory + '/' + tm_save_directory}/z_{rid}", np.array(z_out, dtype=np.float32))
            np.save(
                f"{save_directory + '/' + tm_save_directory}/cg_data_{rid}", np.array(tm_data_out, dtype=np.float32)
            )
            np.save(f"{save_directory + '/' + oms_save_directory}/z_{rid}", np.array(z_out, dtype=np.float32))
            np.save(
                f"{save_directory + '/' + oms_save_directory}/cg_data_{rid}", np.array(oms_data_out, dtype=np.float32)
            )
            np.save(f"{save_directory + '/' + cross_save_directory}/z_{rid}", np.array(z_out, dtype=np.float32))
            np.save(
                f"{save_directory + '/' + cross_save_directory}/cg_data_{rid}", np.array(cross_data_out, dtype=np.float32)
            )
            np.save(f"{save_directory + '/' + signal_store}/z_{rid}", np.array(z_out, dtype=np.float32))
            np.save(
                f"{save_directory + '/' + signal_store}/cg_data_{rid}", np.array(signal_data_out, dtype=np.float32)
            )
