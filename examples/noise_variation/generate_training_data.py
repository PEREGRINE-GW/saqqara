...
def total_sims():
    ...
    count = 0
    files = glob.glob("simulations/cg_data_*.npy")
    for fl in files:
        data = np.load(fl)
        count += data.shape[0]
    return count
...
if __name__ == "__main__":
    ...
    try:
        while total_sims() < 500_000:
            ...
            rid = id_generator(4)
            np.save(f"simulations/z_{rid}", np.array(z_out, dtype=np.float32))
            np.save(f"simulations/cg_data_{rid}", np.array(data_out, dtype=np.float32))
