...
def total_sims():
    ...
    count = 0
    files = glob.glob("coverage_simulations/cg_data_*.npy")
    for fl in files:
        data = np.load(fl)
        count += data.shape[0]
    return count
...
if __name__ == "__main__":
    ...
    try:
        while total_sims() < 20_000:
            ...
            if not os.path.exists("./coverage_simulations/"):
                os.mkdir("./coverage_simulations")
            rid = id_generator(4)
            np.save(f"coverage_simulations/z_{rid}", np.array(z_out, dtype=np.float32))
            np.save(f"coverage_simulations/cg_data_{rid}", np.array(data_out, dtype=np.float32))
