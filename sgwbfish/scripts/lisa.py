...
lisa = sf.LISA()
freq_grid = jnp.geomspace(3e-5, 5e-1, 10_000)
channel = "AA"

if False:
    fig = plt.figure(figsize=(12, 5))
    ax = plt.subplot(1, 2, 1)
    plt.loglog(freq_grid, signal.evaluate(freq_grid, jnp.array([-11.0])))
    plt.loglog(
        freq_grid, lisa.evaluate(freq_grid, jnp.array([3.0, 15.0]), channel=channel)
    )
    plt.xlim(freq_grid[0], freq_grid[-1])
    plt.xlabel(r"$f \mathrm{[Hz]}$")
    plt.ylabel(r"$S(f)$")

    ax = plt.subplot(1, 2, 2)
    plt.loglog(freq_grid, signal.gradient(freq_grid, jnp.array([-11.0])))
    plt.loglog(freq_grid, lisa.gradient(freq_grid, jnp.array([3.0, 15.0])))
    plt.xlim(freq_grid[0], freq_grid[-1])
    plt.xlabel(r"$f \mathrm{[Hz]}$")
    plt.ylabel(r"$\partial S(f)/\partial\theta$")
    plt.tight_layout()
    plt.show()
...
start_time = time.time()
theta_sample = jnp.vstack([get_theta_varying() for _ in range(10)])
covariance_matrices = fish.get_covariance_matrix_vmap(theta_sample)
end_time = time.time()
execution_time = end_time - start_time
print("Execution time:", execution_time, "seconds")
...
covariance_matrices = fish.get_covariance_matrix_vmap(theta_sample)[:, :2, :2]
alpha_sensivity_sample = jnp.sqrt(covariance_matrices[:, 0, 0])
end_time = time.time()
execution_time = end_time - start_time
print("Execution time:", execution_time, "seconds")
...
if True:
    ...
    for i in tqdm.tqdm(range(Nsamples // 10)):
        plot_cov_ellipse(
    ...
    ax.axhline(-beta_sensivity, color=colors["blue"], linewidth=0.7, zorder=-10)
    ax.axhline(beta_sensivity, color=colors["blue"], linewidth=0.7, zorder=-10)
    ax.axhline(-2 * beta_sensivity, color=colors["blue"], linewidth=0.4, zorder=-10)
    ax.axhline(2 * beta_sensivity, color=colors["blue"], linewidth=0.4, zorder=-10)
    ...
    ax_inset.set_xlabel(r"$\sigma(\alpha)$", fontsize=22)
    ax_inset.hist(
        alpha_sensivity_sample,
        bins=20,
        density=True,
        alpha=0.7,
        color=colors["blue"],
    )
    ax_inset.axvline(alpha_sensivity, color=colors["pink"])
    ax_inset.locator_params(axis="both", nbins=5)

    ax = plt.subplot(2, 2, 4)
    ax_inset = ax.inset_axes([0.3, 1.2, 0.7, 0.4])
    ax_inset.set_xlabel(r"$\sigma(\gamma)$", fontsize=22)
    ax_inset.hist(
        np.sqrt(covariance_matrices[:, 1, 1]),
        bins=20,
        density=True,
        alpha=0.7,
        color=colors["blue"],
    )
    ax_inset.axvline(beta_sensivity, color=colors["pink"])
    ax_inset.locator_params(axis="both", nbins=5)
